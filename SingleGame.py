	...
	def update(self):
		with self.renderer:
			...
			if self.after_game_stats == False:
				...
				if self.game_over:
					self.renderer.draw_board(self.previous_board, self.tetromino, self.cur_level, C.BBC)
					self.back_to_menu = self.renderer.draw_game_over_animation(self.surface, self.board, self.block_rect, C.BBC, C.BBC, 
				...
				if self.game_start:
					countdown = self.renderer.draw_start_game_animation(self.surface, self.board, self.block_rect, C.BBC, C.BBC, self.game_start_timer)
					if countdown == False:
						self.fall_speed_timer = 0
